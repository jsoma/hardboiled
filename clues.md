# Hardboiled Clues

These are all of the technologies that Hardboiled can detect. This list is automatically generated from the `clues` directory.

Hardboiled currently supports 88 different clues.

### Amazon CloudFront [http://aws.amazon.com/cloudfront/](http://aws.amazon.com/cloudfront/)

Tagged with: none | Test types: header | Found at: [caching-hosting-cdn/cloudfront.js](https://github.com/jsoma/hardboiled/blob/master/clues/caching-hosting-cdn/cloudfront.js)

### Angular [http://angularjs.org/](http://angularjs.org/)

Tagged with: none | Test types: global, filename | Found at: [framework/angular.js](https://github.com/jsoma/hardboiled/blob/master/clues/framework/angular.js)

### Backbone [http://documentcloud.github.io/backbone/](http://documentcloud.github.io/backbone/)

Tagged with: none | Test types: global | Found at: [uncategorized/backbone.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/backbone.js)

### Blogger [http://www.blogger.com](http://www.blogger.com)

Blog publishing service owned by Google, a.k.a. blogspot.com

Tagged with: none | Test types: meta | Found at: [cms/blogger.js](https://github.com/jsoma/hardboiled/blob/master/clues/cms/blogger.js)

### Bootstrap [http://getbootstrap.com](http://getbootstrap.com)

Tagged with: frameworks | Test types: filename | Found at: [framework/bootstrap.js](https://github.com/jsoma/hardboiled/blob/master/clues/framework/bootstrap.js)

### canvg [http://code.google.com/p/canvg/](http://code.google.com/p/canvg/)

Tagged with: none | Test types: global | Found at: [uncategorized/canvg.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/canvg.js)

### CartoDB [http://www.cartodb.com](http://www.cartodb.com)

Tagged with: none | Test types: selector | Found at: [maps-viz-data/cartodb.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/cartodb.js)

### CartoDB.js [http://developers.cartodb.com/documentation/cartodb-js.html](http://developers.cartodb.com/documentation/cartodb-js.html)

Tagged with: none | Test types: global | Found at: [maps-viz-data/cartodb-js.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/cartodb-js.js)

### Chartbeat [https://chartbeat.com](https://chartbeat.com)

Real-time data for front-line doers

Tagged with: none | Test types: filename | Found at: [analytics/chartbeat.js](https://github.com/jsoma/hardboiled/blob/master/clues/analytics/chartbeat.js)

### CloudFlare [http://www.cloudflare.com](http://www.cloudflare.com)

Tagged with: none | Test types: header | Found at: [caching-hosting-cdn/cloudflare.js](https://github.com/jsoma/hardboiled/blob/master/clues/caching-hosting-cdn/cloudflare.js)

### CloudMade [http://www.cloudmade.com](http://www.cloudmade.com)

Tagged with: none | Test types: selector | Found at: [maps-viz-data/cloudmade.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/cloudmade.js)

### D3 [http://d3js.org](http://d3js.org)

Tagged with: none | Test types: global | Found at: [maps-viz-data/d3.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/d3.js)

### EdgeCast [http://www.edgecast.com/](http://www.edgecast.com/)

Tagged with: none | Test types: header | Found at: [caching-hosting-cdn/edgecast.js](https://github.com/jsoma/hardboiled/blob/master/clues/caching-hosting-cdn/edgecast.js)

### englewood.js [https://github.com/tmcw/englewood.js](https://github.com/tmcw/englewood.js)

Tagged with: none | Test types: global | Found at: [maps-viz-data/englewood.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/englewood.js)

### Facebook Like button

Tagged with: none | Test types: selector | Found at: [social/facebook-like-button.js](https://github.com/jsoma/hardboiled/blob/master/clues/social/facebook-like-button.js)

### Facebook Open Graph tags [https://developers.facebook.com/docs/opengraph/](https://developers.facebook.com/docs/opengraph/)

Tagged with: none | Test types: meta | Found at: [social/facebook-open-graph.js](https://github.com/jsoma/hardboiled/blob/master/clues/social/facebook-open-graph.js)

### Font Awesome [http://fontawesome.io](http://fontawesome.io)

The iconic font designed for Bootstrap (...that's a pun about icons.)

Tagged with: none | Test types: filename | Found at: [uncategorized/font-awesome.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/font-awesome.js)

### Google Analytics [http://www.google.com/analytics](http://www.google.com/analytics)

Tagged with: none | Test types: filename | Found at: [analytics/google-analytics.js](https://github.com/jsoma/hardboiled/blob/master/clues/analytics/google-analytics.js)

### Google Analytics (Doubleclick) [http://www.google.com/analytics](http://www.google.com/analytics)

Tagged with: none | Test types: filename | Found at: [analytics/google-analytics-doubleclick.js](https://github.com/jsoma/hardboiled/blob/master/clues/analytics/google-analytics-doubleclick.js)

### Google Maps (Javascript API) [https://developers.google.com/maps/documentation/javascript/](https://developers.google.com/maps/documentation/javascript/)

Tagged with: none | Test types: global | Found at: [maps-viz-data/google-maps.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/google-maps.js)

### Google Web Fonts [http://www.google.com/fonts](http://www.google.com/fonts)

Tagged with: none | Test types: selector | Found at: [uncategorized/google-web-fonts.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/google-web-fonts.js)

### Handlebars.js [http://handlebarsjs.com](http://handlebarsjs.com)

Minimal templating on steroids. Handlebars provides the power necessary to let you build semantic templates effectively with no frustration.

Tagged with: none | Test types: global | Found at: [templates/handlebars.js](https://github.com/jsoma/hardboiled/blob/master/clues/templates/handlebars.js)

### Highcharts [http://www.highcharts.com](http://www.highcharts.com)

Tagged with: none | Test types: global | Found at: [maps-viz-data/highcharts.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/highcharts.js)

### Highlight.js [http://highlightjs.org](http://highlightjs.org)

Syntax highlighting for the Web

Tagged with: none | Test types: global | Found at: [uncategorized/highlight.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/highlight.js)

### iScroll [http://cubiq.org/iscroll-4](http://cubiq.org/iscroll-4)

Tagged with: none | Test types: javascript | Found at: [uncategorized/iscroll.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/iscroll.js)

### iScroll Lite [http://cubiq.org/iscroll-4](http://cubiq.org/iscroll-4)

Tagged with: none | Test types: javascript | Found at: [uncategorized/iscroll-lite.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/iscroll-lite.js)

### Isotope [http://isotope.metafizzy.co/](http://isotope.metafizzy.co/)

Tagged with: jquery | Test types: jquery | Found at: [jquery/isotope.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/isotope.js)

### jQuery [http://jquery.com](http://jquery.com)

jQuery is a fast, small, and feature-rich JavaScript library.

Tagged with: jquery | Test types: javascript | Found at: [jquery/jquery.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/jquery.js)

### jQuery Address [http://www.asual.com/jquery/address/](http://www.asual.com/jquery/address/)

Tagged with: jquery | Test types: jquery | Found at: [jquery/address.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/address.js)

### jQuery Geocodify [http://datadesk.github.io/jquery-geocodify/](http://datadesk.github.io/jquery-geocodify/)

Autocomplete for address searches

Tagged with: jquery | Test types: jquery | Found at: [jquery/geocodify.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/geocodify.js)

### jQuery HotKeys [https://github.com/jeresig/jquery.hotkeys](https://github.com/jeresig/jquery.hotkeys)

jQuery Hotkeys lets you watch for keyboard events anywhere in your code supporting almost any key combination.

Tagged with: jquery | Test types: jquery | Found at: [jquery/hotkeys.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/hotkeys.js)

### jQuery inview [https://github.com/protonet/jquery.inview](https://github.com/protonet/jquery.inview)

Tagged with: jquery | Test types: javascript | Found at: [jquery/inview.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/inview.js)

### jQuery Sparklines [http://omnipotent.net/jquery.sparkline/](http://omnipotent.net/jquery.sparkline/)

Tagged with: jquery | Test types: jquery | Found at: [jquery/sparkline.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/sparkline.js)

### jQuery UI [http://jqueryui.com](http://jqueryui.com)

Tagged with: jquery | Test types: filename | Found at: [jquery/jquery-ui.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/jquery-ui.js)

### jQuery UI Datepicker [http://api.jqueryui.com/datepicker/](http://api.jqueryui.com/datepicker/)

Tagged with: jquery | Test types: jquery | Found at: [jquery/datepicker.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/datepicker.js)

### Leaflet markercluster [https://github.com/Leaflet/Leaflet.markercluster](https://github.com/Leaflet/Leaflet.markercluster)

Provides Beautiful Animated Marker Clustering functionality for Leaflet, a JS library for interactive maps.

Tagged with: none | Test types: global | Found at: [maps-viz-data/leaflet-markercluster.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/leaflet-markercluster.js)

### Leaflet.js [http://leafletjs.com](http://leafletjs.com)

An Open-Source JavaScript Library for Mobile-Friendly Interactive Maps

Tagged with: none | Test types: global | Found at: [maps-viz-data/leaflet.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/leaflet.js)

### MapBox [http://www.mapbox.com](http://www.mapbox.com)

Tagged with: none | Test types: selector | Found at: [maps-viz-data/mapbox.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/mapbox.js)

### MapBox.js (Leaflet addition) [https://www.mapbox.com/mapbox.js/](https://www.mapbox.com/mapbox.js/)

Tagged with: none | Test types: global | Found at: [maps-viz-data/mapbox-js.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/mapbox-js.js)

### MarkerClustererPlus [http://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclustererplus/docs/reference.html](http://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclustererplus/docs/reference.html)

The library creates and manages per-zoom-level clusters for large amounts of markers.

Tagged with: none | Test types: global | Found at: [uncategorized/markermanagerplus.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/markermanagerplus.js)

### Masonry [http://masonry.desandro.com/](http://masonry.desandro.com/)

Tagged with: none | Test types: global | Found at: [uncategorized/masonry.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/masonry.js)

### Meteor [https://www.meteor.com/](https://www.meteor.com/)

Meteor is a node.js application framework

Tagged with: none | Test types: global | Found at: [framework/meteor.js](https://github.com/jsoma/hardboiled/blob/master/clues/framework/meteor.js)

### miniMonth [https://github.com/tbarmann/jquery.minimonth.js](https://github.com/tbarmann/jquery.minimonth.js)

Tagged with: jquery | Test types: jquery | Found at: [jquery/minimonth.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/minimonth.js)

### Miso [http://misoproject.com/](http://misoproject.com/)

Tagged with: none | Test types: global | Found at: [uncategorized/miso.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/miso.js)

### Miso Dataset [http://misoproject.com/dataset/](http://misoproject.com/dataset/)

Tagged with: none | Test types: global | Found at: [uncategorized/miso-dataset.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/miso-dataset.js)

### Modernizr [http://modernizr.com](http://modernizr.com)

Modernizr is a JavaScript library that detects HTML5 and CSS3 features in the user’s browser.

Tagged with: none | Test types: global | Found at: [uncategorized/modernizr.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/modernizr.js)

### Mollom [http://mollom.com](http://mollom.com)

Tagged with: captcha | Test types: filename | Found at: [uncategorized/mollom.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/mollom.js)

### Mustache.js [https://github.com/janl/mustache.js/](https://github.com/janl/mustache.js/)

Minimal templating with {{mustaches}} in JavaScript

Tagged with: none | Test types: global | Found at: [templates/mustache.js](https://github.com/jsoma/hardboiled/blob/master/clues/templates/mustache.js)

### New Relic Real User Monitoring [https://docs.newrelic.com/docs/features/real-user-monitoring](https://docs.newrelic.com/docs/features/real-user-monitoring)

Tagged with: none | Test types: global | Found at: [uncategorized/newrelic-user-monitoring.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/newrelic-user-monitoring.js)

### Popcorn.js [http://popcornjs.org](http://popcornjs.org)

Tagged with: none | Test types: global | Found at: [uncategorized/popcorn.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/popcorn.js)

### Prototype [http://www.prototypejs.org](http://www.prototypejs.org)

Tagged with: none | Test types: global | Found at: [uncategorized/prototype.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/prototype.js)

### queue.js [https://github.com/mbostock/queue](https://github.com/mbostock/queue)

Queue.js is yet another asynchronous helper library for JavaScript.

Tagged with: none | Test types: javascript | Found at: [uncategorized/queue.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/queue.js)

### RackCache

Tagged with: none | Test types: header | Found at: [caching-hosting-cdn/rack-cache.js](https://github.com/jsoma/hardboiled/blob/master/clues/caching-hosting-cdn/rack-cache.js)

### Ractive.js [http://www.ractivejs.org](http://www.ractivejs.org)

Tagged with: none | Test types: global | Found at: [uncategorized/ractive.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/ractive.js)

### Raphael [http://raphaeljs.com](http://raphaeljs.com)

Raphaël is a small JavaScript library that should simplify your work with vector graphics on the web.

Tagged with: none | Test types: global | Found at: [uncategorized/raphael.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/raphael.js)

### React [http://facebook.github.io/react/](http://facebook.github.io/react/)

A JavaScript library for building user interfaces.

Tagged with: none | Test types: selector | Found at: [uncategorized/react.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/react.js)

### reCAPTCHA [http://www.google.com/recaptcha/](http://www.google.com/recaptcha/)

Tagged with: captcha | Test types: filename | Found at: [uncategorized/recaptcha.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/recaptcha.js)

### reel [http://jquery.vostrel.cz/reel](http://jquery.vostrel.cz/reel)

Tagged with: jquery | Test types: jquery | Found at: [jquery/reel.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/reel.js)

### Require.js [http://requirejs.org](http://requirejs.org)

Tagged with: none | Test types: global | Found at: [uncategorized/requirejs.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/requirejs.js)

### Responsive IFrames [http://npr.github.io/responsiveiframe/](http://npr.github.io/responsiveiframe/)

Tagged with: none | Test types: jquery | Found at: [jquery/responsive-iframes.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/responsive-iframes.js)

### RGB color parser [http://www.phpied.com/rgb-color-parser-in-javascript/](http://www.phpied.com/rgb-color-parser-in-javascript/)

A JavaScript class that accepts a string and tries to figure out a valid color out of it.

Tagged with: none | Test types: javascript | Found at: [uncategorized/rgb-parser.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/rgb-parser.js)

### S3

Tagged with: none | Test types: header | Found at: [caching-hosting-cdn/s3.js](https://github.com/jsoma/hardboiled/blob/master/clues/caching-hosting-cdn/s3.js)

### Select2 [http://ivaynberg.github.io/select2/](http://ivaynberg.github.io/select2/)

Select2 is a jQuery based replacement for select boxes. It supports searching, remote data sets, and infinite scrolling of results.

Tagged with: none | Test types: jquery | Found at: [jquery/select2.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/select2.js)

### Sheetsee.js [https://github.com/jlord/sheetsee.js](https://github.com/jlord/sheetsee.js)

Tagged with: none | Test types: global | Found at: [maps-viz-data/sheetsee.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/sheetsee.js)

### Simple Map D3 [http://code.minnpost.com/simple-map-d3/](http://code.minnpost.com/simple-map-d3/)

Tagged with: none | Test types: global | Found at: [maps-viz-data/simple-map-d3.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/simple-map-d3.js)

### SoundCiteJS [http://soundcite.knightlab.com](http://soundcite.knightlab.com)

Tagged with: none | Test types: global | Found at: [uncategorized/soundcite.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/soundcite.js)

### Stamen Tile Layer [http://maps.stamen.com](http://maps.stamen.com)

Tagged with: none | Test types: javascript | Found at: [maps-viz-data/stamen.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/stamen.js)

### StateFace [http://propublica.github.io/stateface/](http://propublica.github.io/stateface/)

Tagged with: none | Test types: filename | Found at: [uncategorized/stateface.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/stateface.js)

### StoryJS/TimelineJS

Tagged with: none | Test types: global | Found at: [uncategorized/storyjs.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/storyjs.js)

### Tabletop.js [http://github.com/jsoma/tabletop](http://github.com/jsoma/tabletop)

Tabletop.js allows you to use Google Spreadsheets as a JSON-y CMS

Tagged with: javascript | Test types: global | Found at: [uncategorized/tabletop.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/tabletop.js)

### Tangelo [https://github.com/Kitware/tangelo](https://github.com/Kitware/tangelo)

Tagged with: none | Test types: global | Found at: [uncategorized/tangelo.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/tangelo.js)

### three.js [http://threejs.org](http://threejs.org)

JavaScript 3D library

Tagged with: none | Test types: global | Found at: [uncategorized/three.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/three.js)

### Timeline JS, embeddable version [http://timeline.knightlab.com](http://timeline.knightlab.com)

Tagged with: none | Test types: selector | Found at: [uncategorized/timeline-js.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/timeline-js.js)

### TimelineSetter [https://github.com/propublica/timeline-setter](https://github.com/propublica/timeline-setter)

Tagged with: none | Test types: global | Found at: [uncategorized/timeline-setter.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/timeline-setter.js)

### tipsy [https://github.com/jaz303/tipsy](https://github.com/jaz303/tipsy)

Facebook-style tooltips plugin for jQuery

Tagged with: jquery, tooltips | Test types: jquery | Found at: [jquery/tipsy.js](https://github.com/jsoma/hardboiled/blob/master/clues/jquery/tipsy.js)

### Tulip [https://github.com/MinnPost/tulip](https://github.com/MinnPost/tulip)

Tagged with: none | Test types: global | Found at: [maps-viz-data/tulip.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/tulip.js)

### Tumblr [http://tumblr.com](http://tumblr.com)

Tagged with: none | Test types: header | Found at: [cms/tumblr.js](https://github.com/jsoma/hardboiled/blob/master/clues/cms/tumblr.js)

### Twitter Bootstrap, Responsive CSS [http://getbootstrap.com](http://getbootstrap.com)

Prior to Bootstrap 3, you could enable responsive design by adding in an additional stylesheet.

Tagged with: none | Test types: filename | Found at: [framework/bootstrap-responsive.js](https://github.com/jsoma/hardboiled/blob/master/clues/framework/bootstrap-responsive.js)

### Twitter card

Tagged with: none | Test types: meta | Found at: [social/twitter-card.js](https://github.com/jsoma/hardboiled/blob/master/clues/social/twitter-card.js)

### Typekit [https://typekit.com/](https://typekit.com/)

Add a line of code to your pages and choose from hundreds of web fonts. Simple, bulletproof, standards compliant, accessible, and totally legal

Tagged with: none | Test types: global | Found at: [fonts/typekit.js](https://github.com/jsoma/hardboiled/blob/master/clues/fonts/typekit.js)

### Underscore [http://underscorejs.org](http://underscorejs.org)

Underscore is a utility-belt library for JavaScript that provides a lot of the functional programming support that you would expect in Prototype.js (or Ruby), but without extending any of the built-in JavaScript objects. It's the tie to go along with jQuery's tux, and Backbone.js's suspenders.

Tagged with: none | Test types: global | Found at: [uncategorized/underscore.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/underscore.js)

### Validate.js [http://rickharrison.github.io/validate.js/](http://rickharrison.github.io/validate.js/)

Lightweight JavaScript form validation library inspired by CodeIgniter. No dependencies, just over 2kb gzipped, and customizable!

Tagged with: none | Test types: global | Found at: [uncategorized/validate.js](https://github.com/jsoma/hardboiled/blob/master/clues/uncategorized/validate.js)

### Varnish

Tagged with: none | Test types: header | Found at: [caching-hosting-cdn/varnish.js](https://github.com/jsoma/hardboiled/blob/master/clues/caching-hosting-cdn/varnish.js)

### W3 Total Cache

Tagged with: none | Test types: text | Found at: [wordpress/w3-total-cache.js](https://github.com/jsoma/hardboiled/blob/master/clues/wordpress/w3-total-cache.js)

### wax [https://www.mapbox.com/wax/](https://www.mapbox.com/wax/)

Tools for improving web maps.

Tagged with: none | Test types: global | Found at: [maps-viz-data/wax.js](https://github.com/jsoma/hardboiled/blob/master/clues/maps-viz-data/wax.js)

### Wordpress [http://www.wordpress.org](http://www.wordpress.org)

Tagged with: none | Test types: meta | Found at: [wordpress/wordpress.js](https://github.com/jsoma/hardboiled/blob/master/clues/wordpress/wordpress.js)

### Wordpress.com [http://www.wordpress.com](http://www.wordpress.com)

Hosted WP

Tagged with: none | Test types: meta | Found at: [wordpress/wordpress.com.js](https://github.com/jsoma/hardboiled/blob/master/clues/wordpress/wordpress.com.js)

### WP-Super-Cache

Tagged with: none | Test types: text | Found at: [wordpress/wp-super-cache.js](https://github.com/jsoma/hardboiled/blob/master/clues/wordpress/wp-super-cache.js)
